...
namespace rage {
    // 3 floats, no padding - used where the par structure literally packs 3 floats.
    struct Vec3 {
        float x, y, z;
    };

    // SIMD-friendly 16-byte-aligned 4-wide vector (w is unused/padding for 3-component data).
...
// Matches the well-known CHandlingData::eHandlingType layout used across GTA V tooling/dumps.
enum eHandlingType : int32_t {
    ...
    HANDLING_TYPE_TRAILER = 6,
    HANDLING_TYPE_CAR = 7,
    HANDLING_TYPE_WEAPON = 8,
    NUM_HANDLING_TYPES = 9,
};

// Only used for sizing atFixedArray<eVehicleModType, 6> below - exact enumerators aren't needed here.
enum eVehicleModType : uint32_t {
    VMT_INVALID = 0xFFFFFFFFu,
};

// Defined by the game's par (parser) system; we never need its contents, only that
// atArray<CAdvancedData> has a fixed pointer/count/size layout regardless of T.
...
public:
    ...
    float fMaxDriveBiasTransfer;                    //0x002C
    float fJumpForceScale;                          //0x0030
    float fIncreasedRammingForceScale;              //0x0034
...
public:
    ...
    atFixedArray<eVehicleModType, 6>            m_WeaponVehicleModType;     //0x0038
    ...
    atFixedArray<sTurretPitchLimits, 12>         m_TurretPitchLimitData;     //0x01A0
...
// Returns the exact byte size of the concrete subhandling type, or 0 if unknown.
// Used to safely clone a CBaseSubHandlingData* without knowing its type at compile time.
inline size_t GetSubHandlingDataSize(eHandlingType type) {
    switch (type) {
        case HANDLING_TYPE_BIKE:             return sizeof(CBikeHandlingData);
        case HANDLING_TYPE_FLYING:           return sizeof(CFlyingHandlingData);
        case HANDLING_TYPE_VERTICAL_FLYING:  return sizeof(CSpecialFlightHandlingData);
        case HANDLING_TYPE_BOAT:             return sizeof(CBoatHandlingData);
        case HANDLING_TYPE_SEAPLANE:         return sizeof(CSeaPlaneHandlingData);
        case HANDLING_TYPE_SUBMARINE:        return sizeof(CSubmarineHandlingData);
        case HANDLING_TYPE_TRAILER:          return sizeof(CTrailerHandlingData);
        case HANDLING_TYPE_CAR:              return sizeof(CCarHandlingData);
        case HANDLING_TYPE_WEAPON:           return sizeof(CVehicleWeaponHandlingData);
        default:                             return 0;
    }
}
